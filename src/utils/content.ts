...
/**
 * Get featured writing posts (for homepage featured section)
 */
async function _getFeaturedPosts(lang?: Language) {
  ...
  // Fallback: pinned posts, then latest
...
/**
 * Get notes collection entries
 */
async function _getNotes(lang?: Language) {
  const currentLang = lang || defaultLocale

  const filteredNotes = await getCollection(
    'notes',
